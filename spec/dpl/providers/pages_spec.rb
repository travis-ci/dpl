describe Dpl::Providers::Pages do
  ...
  describe 'given --deploy_key a2V5Cg==', record: true do
    ...
    it { should have_run '[info] $ ssh -i ~/.dpl/deploy_key -T git@github.com/travis-ci/dpl.git.git' }
    it { should have_run 'ssh -i ~/.dpl/deploy_key -T git@github.com/travis-ci/dpl.git.git' }
    it { should have_run '[info] Pushing to github.com/travis-ci/dpl.git' }
    it { should have_run 'git push --quiet "git@github.com/travis-ci/dpl.git.git" "gh-pages":"gh-pages" > /dev/null 2>&1' }
    it { should have_run_in_order }
  end
end
