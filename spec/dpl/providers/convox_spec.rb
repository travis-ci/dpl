...
describe Dpl::Providers::Convox do
  ...
  describe 'by default', record: true do
    it { is_expected.to have_env CONVOX_HOST: 'console.convox.com' }
    it { is_expected.to have_env CONVOX_PASSWORD: 'pass' }
    it { is_expected.to have_env CONVOX_APP: 'app' }
    ...
    it { is_expected.to have_run '[info] $ convox apps info --rack rack --app app' }
    it { is_expected.to have_run 'convox apps info --rack rack --app app' }
    it { is_expected.to have_run '[info] Building and promoting application ...' }
    it { is_expected.to have_run "convox deploy --rack rack --app app --wait --id --description #{desc}" }
  ...
  describe 'app does not exist, given --create' do
    ...
    it { is_expected.to have_run '[info] Application app does not exist on rack rack. Creating it ...' }
    it { is_expected.to have_run '[info] $ convox apps create app --generation 2 --rack rack --wait' }
    it { is_expected.to have_run 'convox apps create app --generation 2 --rack rack --wait' }
  end

  describe 'app does not exist, given --create --generation 1' do
    let(:exists) { false }

    it { is_expected.to have_run 'convox apps create app --generation 1 --rack rack --wait' }
  end
  ...
  describe 'given --host host' do
    it { is_expected.to have_env CONVOX_HOST: 'host' }
  end
  ...
  describe 'given --update_cli' do
    it { is_expected.to have_run 'convox update' }
  end

  describe 'given --description other' do
    it { is_expected.to have_run 'convox deploy --rack rack --app app --wait --id --description other' }
  end

  describe 'given --env ONE=$one --env TWO=two' do
    it { is_expected.to have_run 'convox env set ONE\=\$one TWO\=two --rack rack --app app --replace' }
  end

  describe 'given --env ONE=$one --env TWO=two --env "THREE=three four five"' do
    it { is_expected.to have_run 'convox env set ONE\=\$one TWO\=two THREE\=three\ four\ five --rack rack --app app --replace' }
  end

  describe 'given --env-names ONE --env-names TWO --env-names THREE' do
    it { is_expected.to have_run 'convox env set ONE\=\$one TWO\=two THREE\=three --rack rack --app app --replace' }
  end
