...
describe Dpl::Providers::Surge do
  ...
  describe 'given --login login --token token --domain domain', record: true do
    ...
    it { is_expected.to have_run '[validate:runtime] node_js (>= 8.8.1)' }
    it { is_expected.to have_run '[npm:install] surge (surge)' }
    it { is_expected.to have_run "surge #{cwd} domain" }
    it { is_expected.to have_run_in_order }
    it { is_expected.to have_env SURGE_LOGIN: 'login' }
    it { is_expected.to have_env SURGE_TOKEN: 'token' }
  end
  ...
  describe 'given --login login --token token' do
    ...
    it { is_expected.to have_run "surge #{cwd}" }
  end
  ...
  describe 'given --login login --token token --domain domain --project ./path' do
    ...
    it { is_expected.to have_run "surge #{cwd}/path domain" }
  end
